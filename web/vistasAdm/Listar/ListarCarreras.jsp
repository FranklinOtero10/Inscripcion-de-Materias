...
%>
<h1 style="color: darkcyan; text-align: center">CARRERAS</h1><hr>
<div class="row">
    <div class="col-md-3">
    ...
    <div class="col-md-4">
        <ul>
            <c:forEach var="i" begin="1" end="${items}">
                <li style="display: inline;"><a class="btn btn-info" href="/INSCRIPCION/ControladorCarreras?accion=Resultado&pags=${i}&regis=${item}">${i}</a> </li>
            </c:forEach>
        </ul>
    </div>
    <div class="col-md-5">
        <form action="ControladorCarreras" style="float: right">
            Mostrar por página:
...
<table class="table table-hover" >
    ...
    <%  List<Carreras> list = dao.listar(paginas, registros);
        Iterator<Carreras> iter = list.iterator();
        Carreras car = null;
        while (iter.hasNext()) {
            car = iter.next(); %>
    <tbody>
        <tr>
            <td class="text-center"><%=car.getId_carrera()%></td>
            <td class="text-center"><%=car.getCodigo_carrera()%></td>
            <td class="text-center"><%=car.getCarrera()%></td>
            <td class="text-center">
                <a class="btn btn-warning" href="#" data-toggle="modal" data-target="#mEditar" onclick="editar('<%=car.getId_carrera()%>','<%=car.getCodigo_carrera()%>','<%=car.getCarrera()%>')"><span class="fas fa-edit" style="color:white"></span></a>
                <a class="btn btn-danger" href="#" data-toggle="modal" data-target="#mEliminar" onclick="eliminar('<%=car.getId_carrera()%>')"><span class="fas fa-trash-alt"></span></a>
            </td>
        </tr>
        <%}%>
    </tbody>                
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="ControladorCarreras" method="post">
                    Código Carrera:<br/>
                    <input class="form-control" type="text" name="txtcodigo" maxlength="20" required><br/>
                    Carrera
                    <input class="form-control" type="text" name="txtcarrera" maxlength="100" required/><br/>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="ControladorCarreras" method="post">
                    Código Carrera: <br/>
                    <input class="form-control txtCod" type="text" name="txtcod" maxlength="20" required><br/>
                    Carrera <br/>
                    <input class="form-control txtCar" type="text" name="txtcar" maxlength="100" required><br/>
                    <input class="txtIdC" type="text" name="txtidcar" hidden><br>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancelar</button>
...
<script>
    editar = function (idC,cod,car) {
        $('.txtIdC').val(idC);
        $('.txtCod').val(cod);
        $('.txtCar').val(car);
    };
    eliminar = function (idC) {
        $('.mId').val(idC);
    };
</script>